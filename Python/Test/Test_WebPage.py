...
class CSETest(BaseTest):
    def test_buy_item(self):
        ...
        price =f"{101.00:.2f}"
        rate=f"{5.00:.2f}"
        item_name = self.driver.find_element(By.XPATH, "//div[@class='caption']//a[contains(text(),'"+itemNameList+"')]")
        self.assertEqual(itemNameList, item_name.text, "Verify name item")

        self.wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//div[@class='image']//img[@title='"+itemNameList+"']"))).click()

        #phone Details Page
        item_name_detail = self.driver.find_element(By.XPATH, "(//h1[normalize-space()='"+itemNameList+"'])[1]")
        ...
        # WebPage check item cart
            #validate price
        price_check = self.driver.find_element(By.XPATH, "(//td[contains(text(),'"+price+"')])[7]")
        ...
        self.assertEqual('$'+str(Calc), self.wait.until(expected_conditions.visibility_of_element_located(
            (By.XPATH, "(//td[normalize-space() = '$"+Calc+"'])[1]"))).text,
                         "Verify price item")

        self.wait.until(expected_conditions.element_to_be_clickable((By.ID, "button-confirm"))).click()

        self.wait.until(expected_conditions.visibility_of_element_located((By.XPATH, "(//h1[normalize-space()='Your order has been placed!'])[1]")))
